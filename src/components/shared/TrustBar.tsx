const badges = [
  {
    ...
    desc: 'Files encrypted in transit',
  ...
  },
  {
    ...
    title: 'Auto-Delete',
    desc: 'Files removed after conversion',
  ...
  },
  {
    ...
    title: 'No Signup',
    desc: 'Free, anonymous conversions',
    ...
    <div className="flex flex-col items-center gap-6 sm:flex-row sm:justify-center sm:gap-12">
      {badges.map((badge) => (
        <div key={badge.title} className="flex items-center gap-3">
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${dark ? badge.darkColor : badge.lightColor}`}>
            {badge.icon}
          </div>
          <div>
            <p className={`text-sm font-semibold ${dark ? 'text-white' : 'text-gray-900'}`}>{badge.title}</p>
            <p className={`text-xs ${dark ? 'text-gray-400' : 'text-text-muted'}`}>{badge.desc}</p>
          </div>
        </div>
      ))}
    </div>
